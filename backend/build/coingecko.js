...
function parseToUnixSeconds(dateStr) {
...
function parseFromDate(dateStr) {
    return parseToUnixSeconds(dateStr);
}
function parseToDate(dateStr) {
    return parseToUnixSeconds(dateStr) + 86400 - 1;
}
export async function fetchTimeseriesByCoin(coinId, from, to) {
    const fromSec = parseFromDate(from);
    const toSec = parseToDate(to);
    const url = `${COINGECKO_API_BASE}/coins/${encodeURIComponent(coinId)}/market_chart/range` +
    ...
    else if (raw?.prices && typeof raw.prices === "object" && Array.isArray(raw.prices.prices)) {
        pricePairs = raw.prices.prices;
    }
    if (!pricePairs)
        throw new Error("Unexpected API response: missing prices");
    return pricePairs.map(([ts, value]) => ({
        ts: new Date(ts).toISOString(),
        value,
    }));
}
const COIN_DETAIL_PARAMS = "localization=false&tickers=false&market_data=true&community_data=false&developer_data=false&sparkline=false";
export async function fetchCoinMarketData(coinId) {
    ...
    if (!md)
        throw new Error("Unexpected API response: missing market_data");
    return {
